...
{{-- Menambahkan kolom Jenis Sampah --}}
<th class="p-2 border text-center">Jenis Sampah</th>
...
{{-- Menampilkan nama jenis sampah dari relasi, dengan penanganan jika relasi kosong --}}
<td class="p-2 border text-center">{{ optional($item->jenisSampah)->nama_sampah }}</td>
...
class="bg-yellow-500 hover:bg-yellow-600 text-white px-2 py-1 text-sm rounded">Edit</a>
                ...
                {{-- Rute Delete (standalone) --}}
                <form action="{{ route('admin.setoran.destroy', $item->id) }}" method="POST" class="inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="bg-red-500 hover:bg-red-600 text-white px-2 py-1 text-sm rounded"
    ...
    @empty
        <tr>
            <td colspan="6" class="p-4 text-center text-gray-500">Belum ada setoran</td>
        </tr>
    @endforelse
</tbody>
